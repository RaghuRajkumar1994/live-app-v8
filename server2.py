...
MACHINE_LIVE_STATUS: Dict[str, Dict[str, str]] = {}
...
def upload_plan():
    ...
    try:
        ...
        plan_data_raw = df.to_dict('records')
        plan_data_processed = []
...
def handle_submit_downtime(data):
    ...
    try:
        ...
        downtime_entry = {
            ...
            't1_terminal_id': data.get('t1_terminal_id', 'N/A'),
            't2_terminal_id': data.get('t2_terminal_id', 'N/A'),
        }
...
def handle_join_machine_room(data):
    ...
    MACHINE_LIVE_STATUS[machine_name] = {
        'status': 'RUNNING', 
        'reason': '', 
        'timestamp': datetime.now().isoformat()
    }
    socketio.emit('live_status_update', MACHINE_LIVE_STATUS)
...
def handle_mark_plan_complete(data):
    ...
    machine_name = data.get('machineName')
    # NEW: Get the specific shift if provided
    shift_to_mark = data.get('shift') 
    ...
    if machine_name in MACHINE_PLANS:
        ...
        if shift_to_mark:
            ...
            for shift_col in ['A', 'B', 'C']:
                # A shift is relevant if the plan has a non-zero, non-empty value for it
                shift_value = target_item.get(shift_col)
                is_relevant = shift_value and str(shift_value).strip() not in ['', '0', '0.0']

                if is_relevant:
                    has_any_shift_column = True
                    # Check if this relevant shift has been marked as 'completed'
                    if target_item.get(f'status_{shift_col.lower()}') != 'completed':
            ...
            target_item['status'] = 'completed'
            # Also mark all individual shifts for UI consistency
            for shift_col in ['A', 'B', 'C']:
                 shift_value = target_item.get(shift_col)
                 is_relevant = shift_value and str(shift_value).strip() not in ['', '0', '0.0']
                 if is_relevant:
                     target_item[f'status_{shift_col.lower()}'] = 'completed'
...
@socketio.on('report_machine_status')
def handle_report_machine_status(data):
    machine_name = data.get('machineName')
...
@socketio.on('request_live_status')
def handle_request_live_status():
    socketio.emit('live_status_update', MACHINE_LIVE_STATUS, room=request.sid)
